...
class HandLandmarkerHelper(
    ...
    var Allfloat: FloatArray = FloatArray( 1*50*258)
...
//        val handCoordinatesList = mutableListOf<FloatArray>()
//        val numKeyPoints = 21
//        val numCoords = 3
//        val inputShape = intArrayOf(1,50,258)
//        val inputBuffer = TensorBuffer.createFixedSize(inputShape, DataType.FLOAT32)
//        val rightHandCoords = FloatArray(numKeyPoints * numCoords)
//        val leftHandCoords = FloatArray(numKeyPoints * numCoords)
//        val PoseCoords = FloatArray(33* 4)
//
        iudaso(result)
    ...
    fun iudasod(result: HandLandmarkerResult) {
        if (result.landmarks().isEmpty()) {
            return
        }

        val leftHandLandmarks = result.landmarks()[0]
        val leftHandCoords = leftHandLandmarks.map { it.x(); it.y(); it.z() }.toFloatArray()

        val rightHandCoords = if (result.landmarks().size > 1 && result.landmarks()[1].isNotEmpty()) {
            val rightHandLandmarks = result.landmarks()[1]
            rightHandLandmarks.map { it.x(); it.y(); it.z() }.toFloatArray()
        } else {
            FloatArray(leftHandCoords.size) // 初始化一个与左手关键点坐标数组相同大小的全0数组
        }
        Log.e("TestMessage", "initC: " + leftHandCoords.contentToString())
        Log.e("TestMessage", "initD: " + rightHandCoords.size)
        val twoHandsCoords = leftHandCoords.plus(rightHandCoords)
        handCoordinatesList.add(twoHandsCoords)

        if (handCoordinatesList.size >= 50) {
            val flattened = handCoordinatesList.take(50).flatMap { it.toList() }.toFloatArray()
            Log.e("TestMessage", "initA: " + flattened.contentToString())
            Log.e("TestMessage", "initB: " + flattened.size)
        }
    }
    fun iudaso( result: HandLandmarkerResult) {
        PoseCoords.fill(0.0f)
        var twoHnadsfloat: FloatArray
